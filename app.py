...
class DriverSafetySystem:
    ...
    def _load_sounds(self):
        ...
        try:
            ...
            print(f"Sound loading failed: {e} — continuing without audio.")
        return sounds
    ...
    def _process_front(self, frame):
        ...
        final_frame, danger_level = self.collision_warner.process(obj_frame, detections)
        return final_frame, lane_alert, detections, counts, danger_level

    def _process_driver(self, frame):
        result_frame, drowsy_alert, yawn_alert = self.drowsiness_detector.process(frame)
    ...
    def _build_dashboard(self, front_frame, driver_frame,
        ...
        drv_x = FRONT_W + 10
        drv_y = 60
        dashboard[drv_y:drv_y + DRIVER_H, drv_x:drv_x + DRIVER_W] = driver_resized
        ...
        cv2.rectangle(dashboard, (panel_x, panel_y),
        ...
        row_y = panel_y + 60
        for s in statuses:
            cv2.putText(dashboard, f"{s['label']}:",
                        (panel_x + 10, row_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, COLOR_WHITE, 1)
            cv2.putText(dashboard, f"{s['icon']} {s['text']}",
                        (panel_x + 120, row_y),
        ...
        if counts:
            ...
            cv2.putText(dashboard, f"Objects: {count_str}",
                        (panel_x + 10, panel_y + panel_h - 12),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.48, COLOR_WHITE, 1)

        bar_y = DASH_H - 50
        any_alert = lane_alert or drowsy_alert or yawn_alert or danger_level != "SAFE"
        bar_color = (0, 0, 120) if any_alert else (0, 80, 0)
        cv2.rectangle(dashboard, (0, bar_y), (DASH_W, DASH_H), bar_color, -1)
        ...
        cv2.putText(dashboard, msg, (20, bar_y + 32),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.78, msg_color, 2)
        cv2.putText(dashboard, time.strftime("%H:%M:%S"),
                    (DASH_W - 120, bar_y + 32),
    ...
    def _read_frame(self, cap, source):
        ret, frame = cap.read()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                while True:
                    ...
                    if not ret_f or not ret_d:
                        print("Frame read failed — shutting down.")
                        break
                    ...
                    front_frame, lane_alert, _, counts, danger_level = front_future.result()
                    driver_frame, drowsy_alert, yawn_alert = driver_future.result()
